...
- [x] 1. Scaffold the single-file HTML structure
...
- [x] 2. Implement the Cat SVG character and idle animation
  - [x] 2.1 Embed inline SVG cat composed of ellipses (body/head), triangles (ears), and circles (eyes)
  ...
  - [x]* 2.2 Write unit tests for initial DOM state
...
- [x] 3. Define app state and reaction pool
  - [x] 3.1 Declare the `state` object with all six fields: `petCount`, `happiness`, `lastPetTime`, `lastReactionIndex`, `isAnimating`, `pendingPets`
  ...
  - [x]* 3.2 Write property test for reaction pool constraints (Property 7)
...
- [x] 4. Implement core pure functions
  - [x] 4.1 Implement `updateCounter()` — increment `state.petCount` by 1, update `#pet-counter` text
    - _Requirements: 4.2, 4.4_

  - [x]* 4.2 Write property test for pet counter increment (Property 1)
  ...
  - [x] 4.3 Implement `increaseHappiness()` — add 10, cap at 100 with `Math.min`; set `state.lastPetTime = Date.now()`
    - _Requirements: 5.2_

  - [x]* 4.4 Write property test for happiness increment cap (Property 3)
  ...
  - [x] 4.5 Implement `decayTick()` — decrease `state.happiness` by 1 only when `Date.now() - state.lastPetTime > 3000` and `state.happiness > 0`; floor at 0 with `Math.max`
    - _Requirements: 5.3_

  - [x]* 4.6 Write property test for happiness decay floor (Property 4)
  ...
  - [x] 4.7 Implement `selectReaction()` — select a random reaction from `REACTIONS`; guarantee the selected index differs from `state.lastReactionIndex`; update `state.lastReactionIndex`
    - _Requirements: 3.2, 3.5_

  - [x]* 4.8 Write property test for no-consecutive-repeat reactions (Property 5)
  ...
  - [x]* 4.9 Write property test for reaction in pool bounds (Property 6)
...
- [x] 6. Implement `applyState()` and DOM rendering
  - [x] 6.1 Implement `applyState()` — reflect current state into DOM:
  ...
  - [x]* 6.2 Write property test for happiness meter fill proportion (Property 8)
...
- [x] 7. Implement reaction playback (`playReaction()`) and speech bubble
  ...
  - [x]* 7.2 Write unit tests for speech bubble lifecycle
...
- [x] 8. Implement `handlePet()` and event wiring
  - [x] 8.1 Implement `handlePet(event)` — call `event.preventDefault()`; if `state.isAnimating` increment `state.pendingPets` and return; otherwise call `updateCounter()`, `increaseHappiness()`, `selectReaction()`, `playReaction()`, `applyState()`
  ...
  - [x]* 8.2 Write unit tests for event handling
...
- [x] 9. Implement happiness decay loop
  - [x] 9.1 Wire `decayTick()` to `setInterval` with 500 ms interval inside `DOMContentLoaded`
  ...
  - [x]* 9.2 Write property test for happiness always bounded (Property 2)
...
- [x] 10. Implement responsive layout and visual polish
  - [x] 10.1 Apply Tailwind responsive classes to centre the cat and stack the HUD (counter + meter) for all viewport widths 320 px–1920 px
